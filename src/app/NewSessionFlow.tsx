            ...
            <div data-testid="new-session-overlay" role="dialog" aria-modal="true" className="pointer-events-auto flex w-full max-w-[420px] flex-col overflow-hidden rounded-t-2xl border border-border-default border-b-0 bg-surface shadow-2xl">
              ...
              <ul className="sidebar-scroll flex max-h-[calc(3rem*6+0.25rem*5)] flex-col gap-1 overflow-y-auto px-3 pb-4">
                ...
                {cliOptions.map((option) => {
                  ...
                  return <li key={option.id} className="shrink-0"><button type="button" data-testid={`new-session-cli-${option.id}`} onClick={() => openCli(option)} className="cursor-target flex h-12 w-full items-center gap-2.5 rounded-xl px-2.5 text-left font-pingfang transition-colors hover:bg-surface-hover"><span className="inline-flex size-8 shrink-0 items-center justify-center rounded-lg bg-surface-strong"><Icon size={16} className="size-4" /></span><span className="min-w-0"><span className="block text-[12px] font-semibold text-text-primary">{option.name}</span><span className="block truncate text-[11px] text-text-faint">{option.hint}</span></span></button></li>
                })}
              </ul>
            </div>
          </motion.div>
          ...
          <AnimatePresence>
            {draft && (
              <ModalShell testId="cli-config" width="420px" iconAdapterId={draft.option.adapterId} title={strings.newSession.newCli(draft.option.name)} hint={strings.newSession.configureThenLaunch} onClose={() => setDraft(null)}>
                ...
                <div className="mt-3 flex items-center justify-end gap-2 border-t border-border-subtle px-4 py-3"><button type="button" onClick={() => setDraft(null)} className="rounded-lg px-3 py-1.5 font-pingfang text-[12px] font-medium text-text-muted transition-colors hover:bg-surface-strong hover:text-text-secondary">{strings.common.cancel}</button><button type="button" data-testid="cli-launch" onClick={confirmCli} className="rounded-lg bg-button-primary px-3 py-1.5 font-pingfang text-[12px] font-medium text-button-primary-fg transition-colors hover:bg-button-primary-hover">{strings.newSession.launch}</button></div>
              </ModalShell>
            )}
          </AnimatePresence>
        </>
...
function RuntimeButton({ id, selected, title, hint, onSelect }: { id: CliRuntime; selected: boolean; title: string; hint: string; onSelect: (id: CliRuntime) => void }) {
  return <button type="button" data-testid={`cli-runtime-${id}`} onClick={() => onSelect(id)} className={`cursor-target rounded-xl px-3 py-2.5 text-left transition-colors ${selected ? 'bg-button-primary text-button-primary-fg' : 'bg-button-secondary text-button-secondary-fg hover:bg-button-secondary-hover'}`}><span className="block text-[12px] font-semibold">{title}</span><span className={`mt-0.5 block text-[10px] ${selected ? 'text-text-inverse/60' : 'text-text-faint'}`}>{hint}</span></button>
}
