...
/** Runtime-neutral atomic hook drop consumer used by host and WSL adapters. */
export class HookDropPoller {
    ...
    try {
      ...
      const ordered = await Promise.all(
        names.map(async (name) => ({
          name,
          metadata: await lstat(join(this.dropDir, name))
        }))
      )
      ordered.sort(
        (left, right) =>
      ...
      for (const item of ordered) {
        ...
        try {
          if (!item.metadata.isFile() || item.metadata.isSymbolicLink()) continue
          ...
          console.warn('[hook-drop-poller] ignored invalid drop:', String(error))
        } finally {
      ...
      if (!this.disposed) {
        console.warn('[hook-drop-poller] consume failed:', String(error))
      }
    } finally {
