...
> 状态：**P0–P7 已关门；P8 Android 模拟器公网真实 PTY 检查点与 iOS 本地 bundle 实现/导出检查点已完成（2026-08-21），当前仍处于 P8 设备关门阶段。** P2 已部署到公网 HTTPS/WSS 单副本环境；P3–P5 已证明真实 HRack/PTY 的列表、驾驶和远程新建链；P6/P7 已由安装版 Android release App 完成相机配对、实时列表与真实 PTY 新建；P8 已完成 history/live、解析后 ack、输入、真实软键盘避让/尺寸恢复、键盘开关后旋转、离线 Fcitx5 拼音组合提交、`undrive`、新建后直入终端，以及真实 Claude Code/Codex CLI 的模拟器基础视觉 smoke。iOS 已移除占位页并能由 Metro 导出自包含离线终端资源，但还没有在 UIKit/WKWebView 或 iPhone/iPad 真机运行。Android 物理真机、iOS 真机和两款 CLI 的物理设备复验尚未完成，因此不能宣布 P8 全部关门或进入 P8 之后的阶段。
...
**状态（2026-08-21）：Android 模拟器实现检查点与 iOS bundle 实现/导出检查点完成，P8 尚未全平台关门。** 安装版 release App 已经公网 WSS 驾驶真实 Electron/`AgentSessionRuntime`/ConPTY，复读 history、提交输入、在 xterm 解析后 ack；竖屏、Gboard 英文键盘、Fcitx5 Pinyin 和键盘开关后的稳定横屏分别以 43 × 31、43 × 16、43 × 15、97 × 15 更新唯一 winsize，键盘隐藏后恢复 43 × 31。Fcitx5 逐键组合 `zhongwen` 时 App 草稿与 PTY 均无裸拼音，选择候选后 App 草稿才出现“中文”，点击发送后 PTY 只收到最终中文。返回 `undrive`，并用实测尺寸新建后直入终端。真实会话因 Android WebView WebGL 字形裁切显式降级到 DOM；多次公网 6,000 行 burst 为 43.5–49.4 KiB/s 且截图字形完整。本机真实 Claude Code 2.1.220 与 Codex CLI 0.146.0 也已通过公网链路完成本地命令、字节解析、截图和释放 smoke。iOS 使用同一 runtime 的约 1.04 MB 单文件本地 HTML，内容 hash CSP、零网络执行、Metro iOS asset 导出和 Android 包隔离均已通过；这仍不能替代 WKWebView/iPhone 真机。物理 Android、iOS 真机及两款 CLI 在这些设备上的中文与长输出复验仍按验收第 4 项保留，详见计划的实现记录。
