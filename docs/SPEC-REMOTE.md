...
**状态（2026-08-21）：Android 模拟器实现检查点与 iOS bundle 实现/导出/WebKit 引擎检查点完成，P8 尚未全平台关门。** 安装版 release App 已经公网 WSS 驾驶真实 Electron/`AgentSessionRuntime`/ConPTY，复读 history、提交输入、在 xterm 解析后 ack；竖屏、Gboard 英文键盘、Fcitx5 Pinyin 和键盘开关后的稳定横屏分别以 43 × 31、43 × 16、43 × 15、97 × 15 更新唯一 winsize，键盘隐藏后恢复 43 × 31。Fcitx5 逐键组合 `zhongwen` 时 App 草稿与 PTY 均无裸拼音，选择候选后 App 草稿才出现“中文”，点击发送后 PTY 只收到最终中文。返回 `undrive`，并用实测尺寸新建后直入终端。真实会话因 Android WebView WebGL 字形裁切显式降级到 DOM；多次公网 6,000 行 burst 为 43.5–49.4 KiB/s 且截图字形完整。本机真实 Claude Code 2.1.220 与 Codex CLI 0.146.0 也已通过公网链路完成本地命令、字节解析、截图和释放 smoke。iOS 使用同一 runtime 的约 1.04 MB 单文件本地 HTML，内容 hash CSP、Metro iOS asset 导出和 Android 包隔离均已通过；Chromium 与 Playwright WebKit 又完成乱序 history、并发 live、解析后 ACK、输入回传、零网络和像素画面门禁，并锁定 xterm 隐藏字宽探针不得泄漏 32 个测量用 `W`。Safari/WKWebView 当前从启动即使用 DOM，但这些引擎级证据仍不能替代安装版 WKWebView/iPhone 真机。物理 Android、iOS 真机及两款 CLI 在这些设备上的中文与长输出复验仍按验收第 4 项保留，详见计划的实现记录。
