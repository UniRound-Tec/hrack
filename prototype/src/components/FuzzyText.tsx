...
const FuzzyText: React.FC<FuzzyTextProps> = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Layout box matches glyph bounds; canvas overflows for fuzz effect.
      wrapper.style.width = `${textBoundingWidth}px`;
      wrapper.style.height = `${tightHeight}px`;
  ...
  return (
    <span
      ref={wrapperRef}
      className={`relative inline-block overflow-visible align-top ${className}`.trim()}
    >
      <canvas ref={canvasRef} className="pointer-events-auto absolute left-0 top-0 max-w-none" />
    </span>
  );
};
